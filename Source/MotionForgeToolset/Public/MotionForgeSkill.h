...
public:
	...
	UMotionForgeSkill()
	{
		...
		Instructions = TEXT(
			...
			"downloads are free. Variants are the spend, which is why Variants defaults to one. "
			"Leave it there unless the user has actually asked to choose between takes: raising it "
			"multiplies the bill exactly, so two variants is twice the price of the same motion. "
			"Price the batch before submitting and get their agreement on the number. Once a take "
			"exists it is paid for, so download all of them - reviewing first saves nothing.\n"
			"\n"
